...
RSpec.describe MediaReactionVotePolicy do
  ...
  permissions :update? do
    it('does not allow users') {
      expect(subject).not_to permit(owner, media_reaction_vote)
    }

    it('does not allow anons') {
      expect(subject).not_to permit(nil, media_reaction_vote)
    }
  end
  ...
  permissions :create?, :destroy? do
    ...
    it('does not allow other') {
      expect(subject).not_to permit(other, media_reaction_vote)
    }
  end
end
