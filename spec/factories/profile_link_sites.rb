FactoryBot.define do
  factory :profile_link_site do
    ...
    trait :youtube do
      validate_find do
        '\A(https?://)?(www.)?(youtube.com/)?(?<channel>user|c|channel)?/?(?<username>[a-zA-z0-9_\-]+)\z'
      end
    ...
    trait :steam do
      validate_find do
        '\A(https?://)?(www.)?(steamcommunity.com/id/)?(?<username>[a-zA-Z0-9_\-]+)\z'
      end
    ...
    trait :kickstarter do
      validate_find do
        '\A(https?://)?(www.)?(kickstarter.com/profile/)?(?<username>[a-zA-Z0-9_.\-]+)\z'
      end
    ...
    trait :imdb do
      validate_find do
        '\A(https?://)?(www.)?(imdb.com/)?(user/)?(?<username>[a-zA-z0-9_\-]+)\z'
      end
    ...
    trait :reddit do
      validate_find do
        '\A(https?://)?(www.)?(reddit.com/)?(user/|u/)?(/u/)?(?<username>[a-zA-z0-9_\-]+)\z'
      end
    ...
    trait :trakt do
      validate_find do
        '\A(https?://)?(www.)?(trakt.tv/)?(users/)?(?<username>[a-zA-z0-9_\-]+)\z'
      end
