...
RSpec.describe Group, type: :model do
  ...
  it do
    expect(subject).to have_many(:members).class_name('GroupMember').dependent(:destroy)
  end

  it do
    expect(subject).to have_many(:neighbors).class_name('GroupNeighbor').dependent(:destroy)
                                            .with_foreign_key('source_id')
  end

  it do
    expect(subject).to have_many(:tickets).class_name('GroupTicket').dependent(:destroy)
  end

  it do
    expect(subject).to have_many(:invites).class_name('GroupInvite').dependent(:destroy)
  end

  it do
    expect(subject).to have_many(:reports).class_name('GroupReport').dependent(:destroy)
  end
  ...
  it do
    expect(subject).to have_many(:action_logs).class_name('GroupActionLog').dependent(:destroy)
  end
